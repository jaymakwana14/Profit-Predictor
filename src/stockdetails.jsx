...
const StockTable = ({ onStockSelect }) => {
  ...
  const getLogoSymbol = (symbol) => {
    // Remove .NS suffix and clean the symbol
    const baseSymbol = symbol.split('.')[0].replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
    
    // Special cases for company logos - using more standardized symbols
    const specialCases = {
      'PVRINOX': 'PVR',
  ...
  useEffect(() => {
    const fetchAllLogos = async () => {
      ...
      // Get unique symbols that haven't been fetched yet
      const uniqueSymbols = new Set(
        stocks
          .filter(stock => !logoCache.has(getLogoSymbol(stock.symbol)))
          .map(stock => getLogoSymbol(stock.symbol))
      );
      ...
      try {
        ...
        const batchSize = 10;
        const batches = Array.from(uniqueSymbols).reduce((acc, curr, i) => {
        ...
        for (const batch of batches) {
          const promises = batch.map(symbol =>
            axios.get(`${config.apiBaseUrl}/api/stock/logo/${encodeURIComponent(symbol)}`)
              .then(response => ({ 
                symbol,
        ...
        console.error('Error in logo fetching batch:', error);
      }
    };
  ...
  const fetchIndices = async () => {
    try {
      ...
      if (!response.data || !response.data.nifty50 || !response.data.bankNifty) {
        console.error('Invalid indices data format:', response.data);
        throw new Error('Invalid data received from server');
      }

      console.log('Indices data received:', {
      ...
      console.error('Error fetching indices:', {
  ...
  useEffect(() => {
    ...
    // Fetch data every 5 seconds, indices every 1 second
    const dataInterval = setInterval(() => fetchData(false), 5000);
    const indicesInterval = setInterval(fetchIndices, 3000);  // Changed to 1 second
  ...
  const handleRowClick = async (stock) => {
    ...
    try {
      ...
      // Pass the selected stock data to the parent component
      onStockSelect(stock, historicalData, updatedChartOptions);
    } catch (error) {
